"use strict";
// insert_one.ts
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.insert_one = void 0;
var process_1 = require("process");
function insert_one(db, collectionName, data) {
    return __awaiter(this, void 0, void 0, function () {
        var collection, err_1, collection, err_2, collection, err_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    // Return codes: 0 = success, -1 = invalid data, -2 = something is null, -3 = duplicate entry, -4 = permission denied
                    if (data == null || collectionName == null || db == null) {
                        return [2 /*return*/, -2];
                    }
                    if (!(collectionName == 'Drugs' && isDrugData(data))) return [3 /*break*/, 5];
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    collection = db.collection(collectionName);
                    return [4 /*yield*/, collection.insertOne(data)];
                case 2:
                    _a.sent();
                    return [3 /*break*/, 4];
                case 3:
                    err_1 = _a.sent();
                    if (err_1.code == 11000) {
                        console.log("Duplicate entry");
                        return [2 /*return*/, -3];
                    }
                    if (err_1.code == 8000)
                        return [2 /*return*/, -4];
                    console.error(err_1);
                    (0, process_1.exit)(1);
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/, 0];
                case 5:
                    if (!(collectionName == 'pdb_no_dups' && isPdbNoDups(data))) return [3 /*break*/, 10];
                    _a.label = 6;
                case 6:
                    _a.trys.push([6, 8, , 9]);
                    collection = db.collection(collectionName);
                    return [4 /*yield*/, collection.insertOne(data)];
                case 7:
                    _a.sent();
                    return [3 /*break*/, 9];
                case 8:
                    err_2 = _a.sent();
                    console.error(err_2);
                    (0, process_1.exit)(1);
                    return [3 /*break*/, 9];
                case 9: return [2 /*return*/, 0];
                case 10:
                    if (!(collectionName == 'pdb_seq' && isPdbSeq(data))) return [3 /*break*/, 15];
                    _a.label = 11;
                case 11:
                    _a.trys.push([11, 13, , 14]);
                    collection = db.collection(collectionName);
                    return [4 /*yield*/, collection.insertOne(data)];
                case 12:
                    _a.sent();
                    return [3 /*break*/, 14];
                case 13:
                    err_3 = _a.sent();
                    console.error(err_3);
                    (0, process_1.exit)(1);
                    return [3 /*break*/, 14];
                case 14: return [2 /*return*/, 0];
                case 15: return [2 /*return*/, -1];
            }
        });
    });
}
exports.insert_one = insert_one;
function insert_many(db, collectionName, data) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            // Return codes: 0 = success, -1 = invalid data, -2 = something is null
            if (data == null || collectionName == null || db == null) {
                return [2 /*return*/, -2];
            }
            if (collectionName == 'Drugs') {
                data.forEach(function (element) {
                    if (!isDrugData(element)) {
                        return -1;
                    }
                    else {
                        try {
                            var collection = db.collection(collectionName);
                            collection.insertOne(element);
                        }
                        catch (err) {
                            console.error(err);
                            (0, process_1.exit)(1);
                        }
                    }
                });
            }
            else if (collectionName == 'pdb_no_dups') {
                data.forEach(function (element) {
                    if (!isPdbNoDups(element)) {
                        return -1;
                    }
                    else {
                        try {
                            var collection = db.collection(collectionName);
                            collection.insertOne(element);
                        }
                        catch (err) {
                            console.error(err);
                            (0, process_1.exit)(1);
                        }
                    }
                });
            }
            else if (collectionName == 'pdb_seq') {
                data.forEach(function (element) {
                    if (!isPdbSeq(element)) {
                        return -1;
                    }
                    else {
                        try {
                            var collection = db.collection(collectionName);
                            collection.insertOne(element);
                        }
                        catch (err) {
                            console.error(err);
                            (0, process_1.exit)(1);
                        }
                    }
                });
            }
            else {
                return [2 /*return*/, -1];
            }
            return [2 /*return*/];
        });
    });
}
function isDrugData(data) {
    return (typeof data.drugName === 'string' &&
        typeof data.condition === 'string');
}
function isPdbNoDups(data) {
    return (typeof data.structureId === 'string' &&
        typeof data.classification === 'string' &&
        typeof data.experimentalTechnique === 'string' &&
        typeof data.macromoleculeType === 'string' &&
        typeof data.residueCount === 'number' &&
        typeof data.resolution === 'number' &&
        typeof data.structureMolecularWeight === 'number' &&
        typeof data.crystallizationMethod === 'string' &&
        typeof data.densityMatthews === 'number' &&
        typeof data.densityPercentSol === 'number' &&
        typeof data.pdbxDetails === 'string' &&
        typeof data.phValue === 'number' &&
        typeof data.publicationYear === 'number');
}
function isPdbSeq(data) {
    return (typeof data.structureId === 'string' &&
        typeof data.chainId === 'string' &&
        typeof data.sequence === 'string' &&
        typeof data.residueCount === 'number' &&
        typeof data.macromoleculeType === 'string');
}
